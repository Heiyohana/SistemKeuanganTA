          ...
          {Menus.map((menu, index) => {
            return (
              <React.Fragment key={index}>
                ...
                {menu.spacing && (
                  <span
                    className={`${styles.menusection} flex border-b-2 mt-3 border-blue-400 text-blue-400`}
                  >
                    {menu.section}
                  </span>
                )}
                {/* Buat menu */}
                {menu.title === "Keluar" ? (
                  <li className="flex items-center justify-between gap-x-4 cursor-pointer p-3 pl-4 decoration-none hover:bg-blue-600 hover:text-white">
                    <button
